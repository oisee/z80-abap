...
CLASS zcl_cpm_00_apc IMPLEMENTATION.
  ...
  METHOD if_apc_wsp_extension~on_start.
    ...
    " Check for disk parameter in context
    TRY.
  ...
  METHOD if_apc_wsp_extension~on_message.
    ...
    " Check for special __DISK__= command from JavaScript (silent setup)
    DATA(lv_new_disk) = parse_disk_command( lv_input ).
    IF lv_new_disk IS NOT INITIAL.
      mv_disk = lv_new_disk.
      mo_ccp->set_disk( mv_disk ).
      " Silent - no output (initial setup from JS on connect)
      RETURN.
    ENDIF.

    " Handle Ctrl+D break signal (EOF)
    IF lv_input = '__EOF__'.
      IF mv_running_program = abap_true.
        mv_running_program = abap_false.
        mo_cpm->reset( ).
        send_text( i_message_manager = i_message_manager
                   iv_text = |{ c_crlf }^D - Program terminated{ c_crlf }| ).
      ENDIF.
      send_prompt( i_message_manager ).
      RETURN.
    ENDIF.
    ...
    IF lv_run_program = abap_true AND lv_program_data IS NOT INITIAL.
      ...
      " Load files referenced in arguments (for compilers, etc.)
      IF lv_program_args IS NOT INITIAL.
        ...
        " Strip leading = or , from arg (MS-COBOL uses =filename format)
        ...
        IF lv_arg_file IS NOT INITIAL.
          ...
          ELSE.
            send_text( i_message_manager = i_message_manager
                       iv_text = |[File not found for register: { lv_arg_file }]{ c_crlf }| ).
          ENDIF.
        ENDIF.
      ENDIF.

      " Set up command tail and FCB with program arguments
      mo_cpm->setup_command_tail( lv_program_args ).

      " Debug: show what was set up
      IF lv_program_args IS NOT INITIAL.
        send_text( i_message_manager = i_message_manager
                   iv_text = |[Args: { lv_program_args }]{ c_crlf }| ).
      ENDIF.
  ...
  METHOD get_welcome_banner.
    ...
    " Pad to 39 chars to align with box
  ...
  METHOD parse_disk_command.
    " Check for __DISK__=X command (sent by JavaScript on connect)
  ...
  METHOD load_companion_files.
    ...
    " Try to load companion .DAT file from bin table first, then SMW0
