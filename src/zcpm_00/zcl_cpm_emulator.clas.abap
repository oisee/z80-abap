CLASS zcl_cpm_emulator DEFINITION PUBLIC CREATE PUBLIC.
  PUBLIC SECTION.
    ...
    CONSTANTS: c_bdos_system_reset   TYPE i VALUE 0,
               ...
               c_bdos_search_first   TYPE i VALUE 17,
               c_bdos_search_next    TYPE i VALUE 18,
    ...
    METHODS setup_command_tail IMPORTING iv_args TYPE string.
  ...
  PRIVATE SECTION.
    ...
    METHODS file_size IMPORTING iv_fcb_addr TYPE i
                      RETURNING VALUE(rv_result) TYPE i.
    METHODS file_search_first IMPORTING iv_fcb_addr TYPE i
                              RETURNING VALUE(rv_result) TYPE i.
    METHODS file_search_next RETURNING VALUE(rv_result) TYPE i.
    DATA mv_search_pattern TYPE string.
    DATA mv_search_index TYPE i.
ENDCLASS.
...
CLASS zcl_cpm_emulator IMPLEMENTATION.
  ...
  METHOD setup_command_tail.
    " Set up CP/M command tail at 0x80
    " Format: byte 0 = length, bytes 1-127 = command tail (with leading space)
    ...
    " Build tail (CP/M convention: leading space before args)
    IF iv_args IS NOT INITIAL.
      lv_tail = to_upper( iv_args ).
    ELSE.
      lv_tail = ''.
    ENDIF.

    " Limit to 127 characters
    lv_len = strlen( lv_tail ).
    IF lv_len > 127.
      lv_len = 127.
      lv_tail = lv_tail+0(127).
    ENDIF.

    " Write length byte at 0x80
    mo_bus->write_mem( iv_addr = 128 iv_val = lv_len ).

    " Write tail characters at 0x81+
    lv_i = 0.
    WHILE lv_i < lv_len.
      lv_ch = lv_tail+lv_i(1).
      lv_ord = cl_abap_conv_out_ce=>uccpi( lv_ch ).
      mo_bus->write_mem( iv_addr = 129 + lv_i iv_val = lv_ord ).
      lv_i = lv_i + 1.
    ENDWHILE.

    " Fill rest with zeros up to 0xFF
    WHILE lv_i < 127.
      mo_bus->write_mem( iv_addr = 129 + lv_i iv_val = 0 ).
      lv_i = lv_i + 1.
    ENDWHILE.

    " Also set up default FCB at 0x5C with first filename if present
    IF iv_args IS NOT INITIAL.
      DATA lv_first_arg TYPE string.
      SPLIT iv_args AT space INTO lv_first_arg DATA(lv_rest).
      IF lv_first_arg IS NOT INITIAL.
        " Strip leading = or , (MS-COBOL uses =filename format)
        IF strlen( lv_first_arg ) > 1.
          DATA lv_first_char TYPE c LENGTH 1.
          lv_first_char = lv_first_arg+0(1).
          IF lv_first_char = '=' OR lv_first_char = ','.
            lv_first_arg = lv_first_arg+1.
          ENDIF.
        ENDIF.
        setup_fcb( iv_fcb_addr = 92 iv_filename = lv_first_arg ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD file_open.
    ...
    " Debug: output what we're looking for
    console_output( 91 ).  " [
    DATA lv_i TYPE i.
    DATA lv_ch TYPE i.
    lv_i = 0.
    WHILE lv_i < strlen( lv_filename ).
      DATA lv_c TYPE c LENGTH 1.
      lv_c = lv_filename+lv_i(1).
      lv_ch = cl_abap_conv_out_ce=>uccpi( lv_c ).
      console_output( lv_ch ).
      lv_i = lv_i + 1.
    ENDWHILE.
    console_output( 93 ).  " ]
    console_output( 10 ).  " LF
  ...
  METHOD bdos_call.
    ...
    CASE iv_func.
      ...
      WHEN c_bdos_search_first.
        lv_result = file_search_first( lv_de ).
        mo_core->set_a( lv_result ).

      WHEN c_bdos_search_next.
        lv_result = file_search_next( ).
        mo_core->set_a( lv_result ).
  ...
  METHOD dump_memory.
    ...
    DATA lv_ascii TYPE string.
    DATA lv_ch TYPE c LENGTH 1.
    DATA lv_hex_byte TYPE string.
    ...
    WHILE lv_i < iv_len.
      ...
      IF lv_i MOD 16 = 0.
        IF lv_i > 0.
          rv_dump = rv_dump && |  { lv_ascii }| && cl_abap_char_utilities=>newline.
        ENDIF.
        lv_hex = |{ iv_addr + lv_i ALIGN = RIGHT WIDTH = 4 PAD = '0' }:|.
        lv_ascii = ''.
      ENDIF.

      lv_val = mo_bus->read_mem( iv_addr + lv_i ).
      lv_hex_byte = |{ lv_val ALIGN = RIGHT WIDTH = 2 PAD = '0' }|.
      lv_hex = lv_hex && | { lv_hex_byte }|.

      " ASCII representation
      IF lv_val >= 32 AND lv_val < 127.
        lv_ch = cl_abap_conv_in_ce=>uccpi( lv_val ).
        lv_ascii = lv_ascii && lv_ch.
      ELSE.
        lv_ascii = lv_ascii && '.'.
      ENDIF.
    ...
    " Output last line
    IF lv_ascii IS NOT INITIAL.
      rv_dump = rv_dump && lv_hex && |  { lv_ascii }| && cl_abap_char_utilities=>newline.
    ENDIF.
  ENDMETHOD.

  METHOD file_search_first.
    " BDOS function 17 - Search for First
    " Searches for files matching FCB pattern (supports ? wildcards)
    ...
    lv_pattern = fcb_to_filename( iv_fcb_addr ).
    mv_search_pattern = to_upper( lv_pattern ).
    mv_search_index = 0.

    " Debug output: show search pattern
    console_output( 91 ).  " [
    console_output( 83 ).  " S
    console_output( 58 ).  " :
    lv_i = 0.
    WHILE lv_i < strlen( mv_search_pattern ).
      lv_c = mv_search_pattern+lv_i(1).
      lv_ch = cl_abap_conv_out_ce=>uccpi( lv_c ).
      console_output( lv_ch ).
      lv_i = lv_i + 1.
    ENDWHILE.
    ...
    " Debug: show number of registered files
    console_output( 91 ).  " [
    DATA lv_count TYPE i.
    lv_count = lines( mt_file_data ).
    DATA lv_count_str TYPE string.
    lv_count_str = |{ lv_count }|.
    lv_i = 0.
    WHILE lv_i < strlen( lv_count_str ).
      lv_c = lv_count_str+lv_i(1).
      lv_ch = cl_abap_conv_out_ce=>uccpi( lv_c ).
      console_output( lv_ch ).
      lv_i = lv_i + 1.
    ENDWHILE.
    ...
    " Search through registered files
    LOOP AT mt_file_data INTO ls_file.
      mv_search_index = sy-tabix.
      " Simple pattern match (TODO: support ? wildcards)
      IF ls_file-filename = mv_search_pattern OR mv_search_pattern = '????????.???'.
        lv_found = abap_true.
        EXIT.
      ENDIF.
      " Try matching without extension if pattern has no extension
      IF mv_search_pattern NA '.'.
        DATA lv_name_only TYPE string.
        DATA lv_pos TYPE i.
        lv_pos = find( val = ls_file-filename sub = '.' ).
        IF lv_pos >= 0.
          lv_name_only = ls_file-filename+0(lv_pos).
        ELSE.
          lv_name_only = ls_file-filename.
        ENDIF.
        IF lv_name_only = mv_search_pattern.
          lv_found = abap_true.
          EXIT.
        ENDIF.
      ENDIF.
    ...
    IF lv_found = abap_false.
      rv_result = 255.  " No file found
      RETURN.
    ENDIF.

    " Copy directory entry to DMA address (32-byte directory entry)
    " Format: user (1) + filename (8) + ext (3) + extent (1) + s1 (1) + s2 (1) + rc (1) + alloc (16)
    DATA lv_fname TYPE string.
    DATA lv_fext TYPE string.
    DATA lv_fpos TYPE i.

    " Parse filename
    lv_fname = ls_file-filename.
    lv_fpos = find( val = lv_fname sub = '.' ).
    IF lv_fpos >= 0.
      lv_fext = lv_fname+lv_fpos.
      SHIFT lv_fext LEFT BY 1 PLACES.
      lv_fname = lv_fname+0(lv_fpos).
    ELSE.
      lv_fext = ''.
    ENDIF.

    " Write directory entry at DMA
    mo_bus->write_mem( iv_addr = mv_dma_addr iv_val = 0 ).  " User 0

    " Write filename (8 chars, padded)
    DO 8 TIMES.
      lv_i = sy-index - 1.
      IF lv_i < strlen( lv_fname ).
        DATA lv_char TYPE c LENGTH 1.
        lv_char = lv_fname+lv_i(1).
        lv_ch = cl_abap_conv_out_ce=>uccpi( lv_char ).
      ELSE.
        lv_ch = 32.  " Space
      ENDIF.
      mo_bus->write_mem( iv_addr = mv_dma_addr + 1 + lv_i iv_val = lv_ch ).
    ENDDO.

    " Write extension (3 chars, padded)
    DO 3 TIMES.
      lv_i = sy-index - 1.
      IF lv_i < strlen( lv_fext ).
        lv_char = lv_fext+lv_i(1).
        lv_ch = cl_abap_conv_out_ce=>uccpi( lv_char ).
      ELSE.
        lv_ch = 32.
      ENDIF.
      mo_bus->write_mem( iv_addr = mv_dma_addr + 9 + lv_i iv_val = lv_ch ).
    ENDDO.

    " Fill rest with zeros (extent, s1, s2, rc, allocation)
    DO 20 TIMES.
      lv_i = sy-index - 1.
      mo_bus->write_mem( iv_addr = mv_dma_addr + 12 + lv_i iv_val = 0 ).
    ENDDO.

    rv_result = 0.  " Found - return directory code 0
  ENDMETHOD.

  METHOD file_search_next.
    ...
    lv_start = mv_search_index + 1.

    LOOP AT mt_file_data INTO ls_file FROM lv_start.
      mv_search_index = sy-tabix.
      IF ls_file-filename = mv_search_pattern OR mv_search_pattern = '????????.???'.
        lv_found = abap_true.
        EXIT.
    ...
    IF lv_found = abap_false.
      rv_result = 255.  " No more files
      RETURN.
    ENDIF.

    " Same directory entry format as search_first
    DATA lv_fname TYPE string.
    DATA lv_fext TYPE string.
    DATA lv_fpos TYPE i.
    ...
    mo_bus->write_mem( iv_addr = mv_dma_addr iv_val = 0 ).
